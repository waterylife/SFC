...
#ifndef __ESF_NET_DRIVER_SELECT_H__
#define __ESF_NET_DRIVER_SELECT_H__
...
namespace esf{namespace net{	//	namespace

typedef int fd_t;

class CDriverSelect : public esf::net::CDriver<fd_t>
{
public:
	CDriverSelect(){}
	...
	virtual void drive();
	virtual void add(esf::sys::ptr< CReactor<fd_t> > reactor);
	virtual void del(fd_t fd);

protected:
	std::deque< esf::sys::ptr< CReactor<fd_t> > > _reactors;
	
	static const int C_SELECT_WAIT_TIMEOUT	= 200000;
	static const int C_SELECT_MAX_FD		= 1024;
	typedef std::deque< esf::sys::ptr< CReactor<fd_t> > >::iterator deque_it;
};
...
inline void CDriverSelect::add(esf::sys::ptr< CReactor<fd_t> > reactor)
{
	fd_t fd = reactor->key();
	if (fd > C_SELECT_MAX_FD)
		throw esf::sys::bt_ex("select fd too big");
...
inline void CDriverSelect::drive()
{
	...
	for(deque_it it = _reactors.begin(); it != _reactors.end(); it++)
	{
		esf::sys::ptr< CReactor<fd_t> > reactor = *it;
		fd_t fd = reactor->key();
	...
	//	get select result
	std::deque< esf::sys::ptr< CReactor<fd_t> > > restart_q;
	for(deque_it it = _reactors.begin(); it != _reactors.end(); it++)
	{
		fd_t fd = (*it)->key();
		int mask = 0;
