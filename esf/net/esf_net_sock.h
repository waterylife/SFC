...
#ifndef __ESF_NET_SOCK_H__
#define __ESF_NET_SOCK_H__
...
class socket_error: public esf::sys::bt_ex
{public:socket_error(const std::string& err_str) : esf::sys::bt_ex(err_str){}};
...
inline int CSocket::get_sockerror()
{
	...
	if (::getsockopt(_socket_fd, SOL_SOCKET, SO_ERROR, &error, (socklen_t*)&optlen) < 0)
		throw socket_error(strerror(errno));
	return error;
}
